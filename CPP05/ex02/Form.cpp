...
/*   Updated: 2022/08/25 06:23:27 by ayblin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Form::Form(const std::string& name,const std::string& target, const unsigned int EchelonToSign, const unsigned int EchelonToExec)
	: _name(name),_target(target), _signed(false), _EchelonToSign(EchelonToSign), _EchelonToExec(EchelonToExec)
{
	if (EchelonToSign < 1 || EchelonToExec < 1)
		throw Form::GradeTooHighException();
	else if (EchelonToSign > 150 || EchelonToExec > 150)
...
{
	o <<GRN<< "Formulaire :\n" <<"	Nom		:"<< BYELLOW <<rhs.getname()<<NRM<<std::endl;
	o <<GRN<<"	signer  	:"<<BYELLOW <<rhs.getsigned()<<std::endl;
	o <<GRN<<"	EchelonToSign	:"<<BYELLOW << rhs.getEchelonToSign()<<std::endl;
	o <<GRN<<"	EchelonToExec	:"<<BYELLOW << rhs.getEchelonToExec()<<NRM<<std::endl;
	return o;
}
